...
export default function OrganizerScreen() {
    ...
    const [viewItem, setViewItem] = useState<Organizer | null>(null);
        ...
        useCallback(() => {
            const fetchData = async () => {
                try {
                    ...
                    console.error("Error al cargar organizers:", error);
                }
            };
    ...
    const handleEdit = (item: Organizer) => {
        navigation.navigate('OrganizerUpdate', { id: Number(item.id) });
    };

    const handleDelete = async (item: Organizer) => {
        try {
            await OrganizerService.delete(Number(item.id));
            setOrganizers(prev => prev.filter(cat => cat.id !== item.id));
            Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        } catch (error) {
            console.error(error instanceof Error ? error.message : 'Error al eliminar Organizer');
            Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
        }
    };

    const handleView = (item: Organizer) => {
            ...
            <View className="flex-1 px-4" style={globalStyles.border}>
                ...
                <View className="px-4">
                    ...
                    <View className="flex-row justify-between items-center mb-4">
                        ...
                        <TouchableOpacity
                            onPress={() => {
                                navigation.navigate('OrganizerCreate')
                                Haptics.selectionAsync();
                            }}
                    ...
                    <View className="mt-3 max-h-[430px]">
                        <FlatList
                            data={organizers}
                            ListEmptyComponent={<EmptyList message="No hay Organizadores" />}
